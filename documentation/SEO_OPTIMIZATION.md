...
## Portfolio of Alif Ahmad Mukhtar D.H.
...
- **Descriptive Page Title**: Changed `<title>` from "Alif Ahmad" to `Alif Ahmad Mukhtar D.H. | Frontend Engineer & Web Developer` to target highly relevant search terms.
...
1. **Person Schema**:
   - Defines **Alif Ahmad Mukhtar D.H.** as a *Frontend Engineer*.
...
- **Hero Section**: Retained the single `<h1>` tag exclusively for the name (`Alif Ahmad Mukhtar D.H.`).
...
- **Descriptive Alt Text**:
  - Replaced generic alt texts like `alt="Hero Image"` and `alt="Logo"` with descriptive keywords like `alt="Alif Ahmad Mukhtar D.H. - Frontend Engineer"` and `alt="Alif Ahmad - Home"`.
- **Lazy Loading**:
